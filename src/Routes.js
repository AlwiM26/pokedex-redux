  ...
  return (
    <Tab.Navigator 
      screenOptions={({route}) => ({
        ...
        tabBarIcon: ({ color }) => {
          if (route.name === "Home") 
            return <MaterialCommunityIcons name="pokeball" size={25} color={color}/>
          else 
    ...
    >
      <Tab.Screen name="Home" component={PokemonRoutes} options={{headerShown: false, title: "Pokédex"}}/>
      <Tab.Screen name="Favorite" component={FavoriteRoutes} options={{headerShown: false, title: "Favorite"}}/>
    </Tab.Navigator>
  );
}
